...
To set you working directory, use the `setwd()` command. We want to set the working directory to the same location as the file 
```{r, eval=FALSE}
setwd("~/Documents/AARUG")
```

Caution: Knitr doesn't like `setwd()`. It assumes that all data files are under the same directory as the script files 
so you can use relative paths. If you don't like this, you can set the root directory in the global chunk options like this
```{r, eval=FALSE}
opts_chunk$set(root.dir = '/mydir')
```
...
1. The function `nrow()` on a `data.frame` returns the number of rows. Use it,
   ...
   `data_by_2` that includes every other row of the cats data frame
...
## Question: How can you recreate this table but by having "control"
